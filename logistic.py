...
logistic_df.head()
#feature selection using skitlearn algorithm
logistic_df = logistic_df[['tenure', 'age', 'address', 'income',
'ed', 'employ', 'equip',   'callcard', 'wireless','churn']]
logistic_df['churn'] = logistic_df['churn'].astype('int')
logistic_df.head()
#defining X,y
X = np.asarray(logistic_df[['tenure', 'age', 'address', 'income', 'ed', 'employ', 'equip']])
X[0:5]
y = np.asarray(logistic_df['churn'])
y [0:5]
#normalize dataset
from sklearn import preprocessing
...
#build model using LogisticRegression from Scikit-learn package
...
#log loss
#Log loss for evaluation. In logistic regression, the output can be the probability
#of customer churn is yes (or equals to 1). This probability is a value 
